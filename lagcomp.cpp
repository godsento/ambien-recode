...
bool LagCompensation::StartPrediction(AimPlayer* data) {
	...
	int lag = (size <= 2) ? game::TIME_TO_TICKS(record->m_lag_time) : game::TIME_TO_TICKS(data->m_records[1]->m_lag_time);

	// pretty sure that's how it has to be done?
	// edit: fix me if it doesnt work
	lag -= 1;
...
void LagCompensation::PlayerMove(LagRecord* record) {
	...
	if (trace.m_fraction != 1.f) {
		...
		// violance said this was supposed to be 4?
		// idk u should test urself
		for (int i{}; i < 4; ++i) {
